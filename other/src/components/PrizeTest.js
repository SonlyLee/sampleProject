...
class RowItem extends Component{
    render(){
        ...
        return(
            <div className={activedId===content?'row_item row_item-active':'row_item'} id={`row_item_${content}`}>
                {content}
            </div>
        )
    }
...
export default class Prize extends Component {
    ...
    handlePlay() {
        ...
        //随机算出一个动画执行的最小次数，这里可以随机更变数值，按照自己的需求来
        ...
        this.begin = setInterval(() => {
            ...
            if (this.state.activedId === this.state.prizeId && this.state.actTimes > this.state.times) {
                //符合上述所有的条件时才是中奖的时候，两个ID相同并且动画执行的次数大于（或等于也行）设定的最小次数
                ...
                <div className="prize">
                    <div className="prize_container">
                        <div className="container_area">
                            <div className="begin_btn" onClick={() => this.handleBegin()}>点击开始</div>
